...
- **App 2FA (TOTP authenticator + backup codes)** in `auth/mfa.py` / `auth/mfa_routes.py`. **Email OTP was removed as a self-service option** (2026-09-25) — the `/mfa/email/send` route is gone and `verify` rejects `method=email`; users who can't use their authenticator/backup codes contact IT Support. Global on/off via the `mfa_required` runtime setting (currently off — Entra SSO enforces MFA on the primary path); per-user `users.mfa_exempt`; admin reset at `POST /api/v1/users/{id}/mfa/reset`. Enrolment/challenge UI at `/mfa`.
